...
violations['full_address'] = (violations['housenumber'] + ' ' + violations['streetname']).str.strip()
violations['zip'] = violations['zip'].astype(str).str[:5]

# Aggregate violations by address
v_counts = violations.groupby(['full_address', 'zip']).size().reset_index(name='violation_count')
v_counts = v_counts.rename(columns={'full_address': 'address', 'zip': 'zip_code'})

# Also count OPEN violations (severity)
open_v = violations[violations['violationstatus'].astype(str).str.upper() == 'OPEN']
open_counts = open_v.groupby(['full_address', 'zip']).size().reset_index(name='open_violations')
open_counts = open_counts.rename(columns={'full_address': 'address', 'zip': 'zip_code'})

# Merge violations into building data
...
building['violation_count'] = building['violation_count'].fillna(0).astype(int)
building['open_violations'] = building['open_violations'].fillna(0).astype(int)

# Percentile
building['building_percentile'] = (building['complaint_count'].rank(pct=True) * 100).round(0).astype(int)
...
print(f"Buildings with violations data: {(building['violation_count'] > 0).sum()}")
